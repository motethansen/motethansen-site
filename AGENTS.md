...
> Kept in step with `CLAUDE.md` (same facts, Codex framing). If the two ever
> disagree, `CLAUDE.md` is authoritative — update both rather than letting them
> drift, the way `functions/api/writing.js` and the Worker once did.
...
| Agent | Machine | Role |
|---|---|---|
| Codex | MacBook Pro (development only) | Dev agent — code, deploy |
| Claude Code | MacBook Pro (development only) | Dev agent — code, deploy |
